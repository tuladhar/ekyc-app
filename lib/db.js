...
// --- CKYC records (ekyc_applications) ---------------------------------------
...
export async function listRecords(cfg = dbConfig()) {
  return withConnection(cfg, (conn) =>
    exec(conn, cfg.engine, `SELECT ${RECORD_COLUMNS} FROM ekyc_applications ORDER BY id`)
  );
}
...
export async function deleteRecord(id, cfg = dbConfig()) {
  return withConnection(cfg, (conn) =>
    exec(conn, cfg.engine, 'DELETE FROM ekyc_applications WHERE id = $1', [id])
  );
}
